local conceal = require("diagram/conceal")
...
local clear_buffer = function(bufnr)
  ...
  if state.conceal_enable then
    for _, diagram in ipairs(state.diagrams) do
      if diagram.bufnr == bufnr then conceal.clear(diagram) end
    end
  end

  state.diagrams = vim.tbl_filter(function(diagram)
    return diagram.bufnr ~= bufnr
  end, state.diagrams)
end
...
local render_buffer = function(bufnr, winnr, integration)
  ...
  for _, diagram in ipairs(diagrams) do
    ...
    -- conceal
    if state.conceal_enable then conceal.apply(diagram) end
  end
end
...
local setup = function(opts)
  ...
  state.conceal_enable = opts.conceal_enable or state.conceal_enable
